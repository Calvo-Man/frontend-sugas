    ...
    <VCard title="REGISTRO DE PROGRAMAS">
        <VCardText class="d-flex flex-column gap-y-8">
              ...
              <v-form ref="form" v-model="valid">
                ...
                <v-btn v-if="!item" class="mt-3" @click="guardar()" color="primary">Guardar</v-btn>
                <v-btn v-if="item" class="mt-3" @click="editar()" color="primary">Editar</v-btn>
    ...
    async guardar() {
      ...
      if (this.$refs.form.validate()) {
        try {
          ...
          this.$notify({text: 'Programa guardado con éxito...',
          type: 'success'}
    ...
    async editar(){
        try {
         // this.paquete.id  = this.id 
          ...
          this.$notify({text: 'Programa editado con éxito...',
          type: 'success'}
          ) //
          this.resetForm();
          this.$emit('peditar')
